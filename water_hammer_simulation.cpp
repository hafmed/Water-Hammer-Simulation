...
#include "ui_water_hammer_simulation.h"
#include <QDebug>
    ...
    ui(new Ui::water_hammer_simulation)
{
    ...
    ui->doubleSpinBox_LimitUp->show();
    ui->doubleSpinBox_LimitDown->show();
#else
    ui->doubleSpinBox_LimitUp->hide();
    ui->doubleSpinBox_LimitDown->hide();
#endif
    /////---------------
    QString fig_whs_fr ="QLabel{ image: url(:/icons/water_hammer_simulation_Reservoir_fr.png);}";
    /////QPixmap (":/icons/GVF-Eqts-Fr.png");
    QString fig_whs_en = "QLabel{ image: url(:/icons/water_hammer_simulation_Reservoir_ang.png);}";
    ////QPixmap (":/icons/GVF-Eqts-En.png");
    QString locale = QString(QLocale::system().name()).left(2);
    if (locale.length() < 2) locale = "en";
    if (locale!="fr")
    {
        ui->label_fig_config->setStyleSheet(fig_whs_en);
    }else{
        ui->label_fig_config->setStyleSheet(fig_whs_fr);
    }
    /////------10-4-2023--------
    connect(ui->lineEdit_QchemiEquili, SIGNAL(valueChanged(double)), this, SLOT(calculQtoVSurgeTanksYmax()));
    connect(ui->lineEdit_VochemiEquili, SIGNAL(valueChanged(double)), this, SLOT(calculVtoQSurgeTanksYmax()));
    /////---15-3-2024
    ui->horizontalSlider_k->setMaximum(mcolones);
    /////on_comboBox_Anim_Valid_currentIndexChanged(0);
    /// HAF 10-5-2024------------------------
    connect(ui->checkBox_dimensionless_graphic, SIGNAL(stateChanged(int)), this, SLOT(recalcul()));
}
water_hammer_simulation::~water_hammer_simulation()
{
    delete ui;
}
void water_hammer_simulation::readData()
{
    // Entrées
...
void water_hammer_simulation::on_horizontalSlider_k_valueChanged(int k)
{
    index=ui->comboBox_Anim_Valid->currentIndex();
    if (index==1) {
        plotHAF(k);
    }
    if (index==2) {
...
void water_hammer_simulation::plotXHAF(int k)
{
    readData();
    for(i=0;i<nLignes ;i++)
    {
    ...
    ui->horizontalSlider_k->setMaximum(mcolones-1);
    //------------------------------------------------
    QVector<double> x(nLignes), y(nLignes);
    for (i=0; i<nLignes; ++i)
    {
        x[i]=i*daltat;
    ...
    ui->customPlot_X->setInteractions(QCP::iRangeDrag | QCP::iRangeZoom);

    ui->customPlot_X->addGraph();
    ui->customPlot_X->graph(0)->setName(tr("Simulation water_hammer_simulation"));
    ui->customPlot_X->graph(0)->setData(x, y);
    ui->customPlot_X->xAxis->setLabel("t (s)");
    /////ui->customPlot_X->xAxis->setRange(0, 1);
    if (ui->radioButton_H->isChecked()){
        ui->customPlot_X->yAxis->setLabel("H (m)");
        ui->customPlot_X->yAxis->setRange(lowH, highH);
        ui->statusBar->showMessage(minmaxh);
    }else if(ui->radioButton_V->isChecked()){
        ui->customPlot_X->yAxis->setLabel("V (m/s)");
        ui->customPlot_X->yAxis->setRange(lowV, highV);
        ui->statusBar->showMessage(minmaxv);
    }else if(ui->radioButton_p->isChecked()){
        ui->customPlot_X->yAxis->setLabel("P (kPa)");
        ui->customPlot_X->yAxis->setRange(lowp, highp);
        ui->statusBar->showMessage(minmaxp);
    }
    ui->customPlot_X->xAxis->setRange(0, tmax);
    ui->customPlot_X->graph(0)->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCircle, Qt::red, Qt::white, 7));
    ui->customPlot_X->axisRect()->setupFullAxesBox();
    ui->customPlot_X->legend->setVisible(false);

    QString X= "x/L="+QString::number((k*dx)/L, 'f', 2)+" (-)";
    ui->label_tempsSimulation->setText(X);
    ui->label_t_xL->setText("x/L (-)");
    ...
    ui->label_TempsRetour->setText(QString(tr("Temps d'aller-retour de l'onde de pression (2*L/c)=%2 (s)"))
                                       .arg(2*L/c, 0, 'f', 2));
    ui->label_DPtheo_DPnum->setVisible(false);

    ui->customPlot_X->replot();
    pd.close();
}
void water_hammer_simulation::on_comboBox_Anim_Valid_currentIndexChanged(int k)
{
    index=ui->comboBox_Anim_Valid->currentIndex();
    /////-----------HAF 29-8-2025----------------------------
    if (index==0) {
        ui->checkBox_dimensionless_graphic->show();
    }else{
        ui->checkBox_dimensionless_graphic->hide();
    }
    /////----------------------------------------------------
    if (index==0) {
        plotHAFVanne();
        ui->label_t_xL->hide();
        ui->horizontalSlider_k->hide();
        ui->label_tempsSimulation->hide();
        if (ui->radioButton_H->isChecked()){
            ui->statusBar->showMessage(minmaxh);
        }else if(ui->radioButton_V->isChecked()){
            ui->statusBar->showMessage(minmaxv);
    ...
    if (index==1) {
        plotHAF(k);
        ui->label_t_xL->setText("t (s)");
        ui->label_t_xL->show();
        ui->horizontalSlider_k->show();
        ui->label_tempsSimulation->show();
...
void water_hammer_simulation::on_tabWidget_ce_currentChanged(int index)
{
    if (index==2) calculSurgeTanksYmax();
}
void water_hammer_simulation::calculQtoVSurgeTanksYmax()
{
    float  Dconduite=ui->lineEdit_dconduite->value();
    float  Qconduite=ui->lineEdit_QchemiEquili->value();
    ui->lineEdit_VochemiEquili->setValue(Qconduite/(M_PI*Dconduite*Dconduite/4.));
}
void water_hammer_simulation::calculVtoQSurgeTanksYmax()
{
    float  Dconduite=ui->lineEdit_dconduite->value();
    float  VoConduite=ui->lineEdit_VochemiEquili->value();
    ui->lineEdit_QchemiEquili->setValue(VoConduite*(M_PI*Dconduite*Dconduite/4.));
}

void water_hammer_simulation::calculSurgeTanksYmax()
{
    float  Dconduite=ui->lineEdit_dconduite->value();
    float  Dchemintequilibre=ui->lineEdit_DchemiEquili->value();
    /////float  Qconduite=ui->lineEdit_QchemiEquili->value();
    float  Lconduite=ui->lineEdit_LchemiEquili->value();
    float  hL=ui->lineEdit_HLchemiEquili->value();
    float  VoConduite=ui->lineEdit_VochemiEquili->value();
    float  fConduite=ui->lineEdit_f_conduite->value();
    /////calcul

    float  As=M_PI*Dchemintequilibre*Dchemintequilibre/4.;
    float  A=M_PI*Dconduite*Dconduite/4.;
    float  Kf;
    if(ui->checkBox_fce->isChecked())
    {
        Kf=fConduite*Lconduite/(2.*9.81*Dconduite);
    }
    else
    {
        Kf=hL/pow(VoConduite,2);
    }



    float  Betta=(Lconduite*A)/(2.*9.81*Kf*As);
    cout<< "Betta="<<Betta<< endl;
    cout<< "hL="<<hL<< endl;

    ///// Methode de bissection (la methode de pt fixe converge ver la solution negative)
    eps=0.000001;
    xg=0;
    xd=Betta-0.0001;
    ...
    {
        xm=(xd+xg)/2.;
        if((functionfSurgeTanksYmax(xm,Betta,hL)*functionfSurgeTanksYmax(xg,Betta,hL))>0)
        {
            xg=xm;
        }
        else
        ...
        cout<< "xm="<<xm<<" et f(xm)="<< functionfSurgeTanksYmax(xm,Betta,hL)<< endl;

    }

    ui->label_ymax->setText(QString(tr("Ymax=%2 (m)")).arg(xm, 0, 'f', 2));

    cout<< "ymax="<<VoConduite*sqrt((A*Lconduite)/(As*32.2))<< endl;

}
double water_hammer_simulation::functionfSurgeTanksYmax(double ymax, double Betta, double hL)
{
    return Betta*log(Betta/(Betta-ymax))-hL-ymax;
}
...
void water_hammer_simulation::checkDimVYforrecalcul()
{
    ...
    m=int(tmax/daltat);
    QString text_label_dimensionTab;
    if (ui->radioButton_H->isChecked()){
        text_label_dimensionTab="H";
    }else if(ui->radioButton_V->isChecked()){
        text_label_dimensionTab="V";
    }else if(ui->radioButton_p->isChecked()){
        text_label_dimensionTab="P";
    }
    text_label_dimensionTab=text_label_dimensionTab+ "("+QString::number(m)+" , "+QString::number(nparts+1)+")";
    ui->label_dimensionTab->setText(text_label_dimensionTab);
    //cout<<"m="<<endl;
...
{
    index=ui->comboBox_Choisemethod->currentIndex();
    if (index==0){
    ...
    k=ui->horizontalSlider_k->value();
    on_comboBox_Anim_Valid_currentIndexChanged(k);
    affichage_resultats();
}
...
void water_hammer_simulation::StopAnimation()
{
    /////----- HAF Correction  bug crash 5-8-20222----
    if (timer->isActive()){
        timer->stop();
        ui->pushButton_StartAnimation->setText(tr("Commencer l'animation"));
    }
...
void water_hammer_simulation::StartAnimation()
{
    ...
    ui->comboBox_Anim_Valid->setCurrentIndex(1);
...
void water_hammer_simulation::plotHAFVanne()
{
    ...
    if (ui->comboBox_Anim_Valid->currentIndex()==1 || ui->comboBox_Anim_Valid->currentIndex()==2){
    ...
    {
        /////---HAF 10-5-2024-----------------
        if (ui->checkBox_dimensionless_graphic->isChecked()){
            xx[i]=i*daltat/(2*L/c);
        }else{
            xx[i]=i*daltat;
        }
        ...
        if (ui->radioButton_H->isChecked()){
            if (ui->checkBox_dimensionless_graphic->isChecked()){
                if (tclose<2*L/c){
                    yy[i] = ((h[i][nparts]-h[0][nparts])*g)/(c*v0) ;
                }else{
                    yy[i] = ((h[i][nparts]-h[0][nparts])*g)/((2*L/tclose)*v0) ;
                }
            ...
            if (ui->checkBox_dimensionless_graphic->isChecked()){
                if (tclose<2*L/c){
                    yy[i] = (p[i][nparts]-p[0][nparts])/(c*v0) ; //P[][] est en kPa
                }else{
                    yy[i] = (p[i][nparts]-p[0][nparts])/((2*L/tclose)*v0) ; //P[][] est en kPa
                }
            }else{
                yy[i] = p[i][nparts] ;
            }
    ...
    if (ui->checkBox_dimensionless_graphic->isChecked()){
        ...
        if (ui->radioButton_H->isChecked()){
            ...
            LimitDown=lowV2-0.15*fabs(lowV2);
            LimitUp=highV2+0.15*fabs(highV2);
        }else if(ui->radioButton_p->isChecked()){
            LimitDown=-1.25;
            LimitUp=1.25;
        }
        ...
        ui->customPlot_2->yAxis->setRange(LimitDown,  LimitUp);
        ui->doubleSpinBox_LimitDown->setValue(LimitDown);
        ui->doubleSpinBox_LimitUp->setValue(LimitUp);

        if (ui->radioButton_H->isChecked()){
            if (tclose<2*L/c){
                ui->customPlot_2->yAxis->setLabel("DH/((C*Vo)/g) (-)");
            }else{
                ui->customPlot_2->yAxis->setLabel("DH/(((2*L/tclose)*Vo)/g) (-)");
            }
        }else{
            if (tclose<2*L/c){
                ui->customPlot_2->yAxis->setLabel("DP/(Rol*C*Vo)  (-)"); //DP est en kPa
            }else{
                ui->customPlot_2->yAxis->setLabel("DP/(Rol*(2*L/tclose)*Vo)  (-)"); //DP est en kPa
            }
        }
    }else{
        ui->customPlot_2->xAxis->setLabel("t (s)");
        ui->customPlot_2->xAxis->setRange(-tmax*0.25, tmax);
        ...
        if (ui->radioButton_H->isChecked()){
            LimitDown=lowH2-0.5*fabs(lowH2);
            LimitUp=highH2+0.25*fabs(highH2);
        }else if(ui->radioButton_V->isChecked()){
            LimitDown=lowV2-0.15*fabs(lowV2);
            LimitUp=highV2+0.15*fabs(highV2);
        }else if(ui->radioButton_p->isChecked()){
            LimitDown=lowp-0.15*fabs(lowp);
            LimitUp=highp+0.15*fabs(highp);
        }
    ...
    if ((ui->radioButton_H->isChecked() || ui->radioButton_p->isChecked())){
        ...
        textLabel->position->setCoords(0.19,  .81); // place position at center/top of axis rect
        if (ui->radioButton_H->isChecked()){
...
{
    readData();
    QString daltattext=" dt="+QString::number(daltat,'f',3)+" (s)"+" ; dx="+QString::number(dx,'f',3)+" (m)";
...
void water_hammer_simulation::about()
{
    ...
    QString pubabout=tr("Avec différentes méthodes numériques cette application simule le phénomène de coup de Bélier ; \n")+
            ...
            " Qt Ver. " +QT_VERSION_STR+"; Compiler: "+CMP_VERSION;
    msgBox.setText(pubabout);
    msgBox.exec();
}
void water_hammer_simulation::on_doubleSpinBox_LimitUp_valueChanged(double arg1)
{
    LimitUp=arg1;
    ui->customPlot_2->yAxis->setRange(LimitDown,  LimitUp);
    ui->customPlot_2->replot();
...
void water_hammer_simulation::on_doubleSpinBox_LimitDown_valueChanged(double arg1)
{
    LimitDown=arg1;
    ui->customPlot_2->yAxis->setRange(LimitDown,  LimitUp);
    ui->customPlot_2->replot();
